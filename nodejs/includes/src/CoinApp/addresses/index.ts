...
import CoinAppNetworksFactory from './../networks';
...
export default class CoinAppAddressesFactory {
    ...
    async getAddressesByStatus(status : AddressStatuses, format : string | undefined = undefined)
    {
        let result : {
            [format : string] : {
        ...
        if (format !== undefined)
        {
            formatObject = this._networksFactory.getFormat(format);
            if (formatObject === undefined)
            {
                throw new Error('Format not found');
            }

            result[formatObject.format.value] = {};
        }
        else
        {
            for (let f in this._networksFactory.formats)
            {
                result[this._networksFactory.formats[f].format.value] = {};
            }
        }
        ...
        try {
            let [data] = await this._DB.pool.promise().query<Array<RowDataPacket>>("\
                SELECT *\
                FROM `addresses` as m\
                WHERE status = ? "+(format !== undefined ? " AND networkFormat = ?" : "")+"\
            ", [status, formatObject?.format.id]);

            if (data.length > 0)
            {
                for (let it of data)
                {
                    let fObject = this._networksFactory.getFormatById(it.networkFormat);
                    if (fObject)
                    {
                        result[fObject.format.value][it.value] = it.exchangeId;
                    }
                }
            }
